...
export function useUpdateIssue() {
  ...
  return useMutation({
    mutationFn: (v: { id: string; actor: string; patch: UpdatePayload }) =>
      updateIssue(v.id, v.actor, v.patch),
    onSuccess: (fresh, v) => settle(fresh, v.id),
  });
}
...
export function useAddComment() {
  const { settle } = useIssueWrites();
  return useMutation({
    mutationFn: (v: { id: string; actor: string; text: string }) =>
...
export function useAddDep() {
  const { settle } = useIssueWrites();
  return useMutation({
    mutationFn: (v: { id: string; actor: string; dependsOn: string; type: DepType }) =>
