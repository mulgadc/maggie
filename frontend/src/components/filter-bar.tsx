          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-xs whitespace-nowrap ${
              on
                ...
                : "border-transparent text-muted line-through opacity-50"
            }`}
...
export function FilterBar({
  ...
  const matched = useMemo(
    () => applyFilters(all, filters).length,
    [all, filters],
  )
  ...
  return (
    <div className="flex flex-col gap-2 border-b border-line bg-bg px-5 py-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className={`text-sm text-muted ${onGroupMode ? "" : "ml-auto"}`}>
