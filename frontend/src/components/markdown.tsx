...
const components: Components = {
  ...
  code: ({ children, className }) => {
    ...
    return (
      <code className="rounded bg-surface2 px-1 py-0.5 font-mono text-[0.85em]">{children}</code>
    );
  },
  a: ({ children, href }) => (
    <a
  ...
  blockquote: ({ children }) => (
    <blockquote className="mb-2 border-line border-l-2 pl-3 text-muted">{children}</blockquote>
  ),
  table: ({ children }) => (
    <table className="mb-2 w-full border-collapse text-xs">{children}</table>
  ),
  th: ({ children }) => <th className="border border-line px-1.5 py-1 text-left">{children}</th>,
  td: ({ children }) => <td className="border border-line px-1.5 py-1 text-left">{children}</td>,
...
export function Markdown({ text }: { text: string }) {
  return (
    <div className="break-words text-sm text-text/90 leading-relaxed">
