...
function Board() {
  ...
  return (
    <>
      {actor ? null : (
        <p className="mb-3 rounded-md border border-line bg-panel px-3 py-2 text-muted text-xs">
          set your identity in the header (“acting as”) to drag beads between columns.
        </p>
      )}
      <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-3.5">
        {COLUMNS.map((status) => (
          ...
          >
            <h2 className="mb-2 px-1 text-muted text-xs uppercase tracking-widest">
            ...
            {grouped.get(status)?.map((issue) => (
              <div
                ...
                className={dragId === issue.id ? "opacity-50" : actor ? "cursor-grab" : ""}
              >
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
      <div className="w-80 rounded-lg border border-line bg-panel p-4 shadow-2xl">
        <h3 className="font-semibold text-sm">Start work on this bead</h3>
        <p className="mt-1 text-muted text-xs">
          <span className="font-mono text-accent">{claim.issue.id}</span> is unassigned. Assign it
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button
            type="button"
            onClick={onCancel}
            className="rounded-md border border-line px-3 py-1 text-muted text-xs hover:text-text"
          >
            cancel
          </button>
          <button
            type="button"
            onClick={() => onConfirm(assignee)}
            className="rounded-md bg-accent px-3 py-1 font-medium text-bg text-xs hover:bg-accent/90"
          >
            move
          </button>
        </div>
