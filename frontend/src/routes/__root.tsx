  ...
  return (
    <div className="flex h-full flex-col">
      <header className="sticky top-0 z-10 flex items-center gap-4 border-line border-b bg-bg px-5 py-3">
        <h1 className="font-semibold text-amber tracking-wide">⚘ WARATAH</h1>
        <nav className="flex gap-2">
          {TABS.map((t) => (
            <Link
              ...
              className="rounded-md border border-line px-3 py-1.5 text-muted text-sm hover:text-text [&.active]:border-amber [&.active]:bg-amber [&.active]:text-bg"
      ...
      <IssueDetail id={issue ?? ""} onClose={closeDetail} />
    </div>
  );
}
